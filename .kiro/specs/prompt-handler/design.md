...
> ⚠️ **[SUPERSEDED 2026-05-25 by 生命周期重画]**
> §10.2-10.4 的机制(HandledSet 指纹去重 / 两阶段截断 / scrollback-residual 诊断)被 user 从第一性原理否定:
> 判据应看**结果(能否输入/能否干活)**,不看**症状(屏幕文字残不残留)**。§10.3 visible-only capture 仍保留(降噪)。
> §10.1 的 3 个缺陷(Q1/Q2/Q3)定性仍有效,但解法改为:**agent 生命周期主干 + 客观核心判断(can-input 确认探针)+ 通用兜底循环(读屏→查DB/LLM判→操作→记录→复测)+ DB 自学习手册**。
> 新设计见 PR4a/PR4b(锚点:三方收敛 + user 拍板的生命周期主干)。读本节仅作历史参考,**不要再按 §10.2 加 `handled_prompt_signatures`**。
