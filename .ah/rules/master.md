...
- **cargo 政策(2026-07-13 用户定案:把全量 cargo test 拿回本地,但只在模块收口点跑一次)**:实施**迭代期间**只用增量 `cargo check` + 单测试定点复现(防 OOM、防逐 task 浪费);**模块/PR 收口点(push 前)本地跑一次全量 `CARGO_BUILD_JOBS=1 cargo test --workspace --test-threads=1`**,一把拿到完整失败清单、一次改全、再 push。CI 是最终确认门,**不是 call-site / 失败发现器**。
