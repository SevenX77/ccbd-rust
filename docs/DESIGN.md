...
新设计：**物理路径位于 `~/.local/state/ah/<project_id>/`（目录名取项目绝对路径 SHA256 前 8 位，见 state_layout.rs:119），一个 ccbd 实例对应一个活跃项目**。L3 编排层通过 RPC 告诉 ccbd "在项目内启动 agent"，ccbd 统一分配 PTY 并通过 **物理 Outcome 探针** 确保其真正进入可输入状态。
...
4. 同时扫文件系统孤儿：
   - `~/.local/state/ah/<project_id>/sandboxes/` 中没有对应 `agents` 行的 → 标记可清理。
...
| **ccbd 状态目录** | `~/.local/state/ah/<project_id>/` | SQLite db、socket、ccbd 自己的日志 |
| **ccbd 缓存** | `~/.cache/ah/sandboxes/<short_hash>/` | 每个项目的 agent 沙盒 HOME（auth symlink + 隔离的 history）|
...
pub fn resolve_state_layout(request: &StateLayoutRequest) -> StateLayout {
    ...
    // ... 基于项目 ah.toml 位置的哈希路径 ...
}
```

开发：`CCB_ENV=dev cargo run`，所有运行态产物都在 `target/dev_state/` 下，可以放心 `cargo clean` 重置。
部署：`cargo build --release`，二进制复制到 `~/.local/bin/ah`，运行时自动按项目隔离。
