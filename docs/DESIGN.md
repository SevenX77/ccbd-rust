...
### 4.1 全局唯一 ccbd（带有项目隔离的守护进程模型）

旧 Python CCB 是 **per-project ccbd**：每个项目目录下一个 `.ccb/ccbd/`，每次 `cd` 到不同项目 = 不同 ccbd。这是混乱根源。

新设计：**物理路径位于 `~/.local/state/ccb-rs/<project_id>/`（目录名取项目绝对路径 SHA256 前 8 位，见 state_layout.rs:119），一个 ccbd 实例对应一个活跃项目**。L3 编排层通过 RPC 告诉 ccbd "在项目内启动 agent"，ccbd 统一分配 PTY 并通过 **物理 Outcome 探针** 确保其真正进入可输入状态。

类比：Docker daemon。但 ccbd-rust 强制实现了状态与源码的物理隔离。

### 4.2 Source of Truth = SQLite

**坚决使用 SQLite**（[`rusqlite`](https://docs.rs/rusqlite)）。不要内存，不要文件系统拼凑。

#### 4.2.1 表结构（最小起步）

```sql
CREATE TABLE projects (
    id TEXT PRIMARY KEY,                -- 客户端 session.create 时传入的项目标识（当前为目录名，见 start.rs:47）
    absolute_path TEXT NOT NULL UNIQUE, -- /home/sevenx/coding/foo
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
) STRICT;

CREATE TABLE sessions (
    id TEXT PRIMARY KEY,                -- L3 分配的 UUID
    project_id TEXT NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    master_pid INTEGER NOT NULL,
    master_pane_id TEXT,                -- 主控 pane ID (%7)
    status TEXT NOT NULL DEFAULT 'ACTIVE',
    created_at INTEGER NOT NULL DEFAULT (unixepoch())
) STRICT;

CREATE TABLE agents (
    id TEXT PRIMARY KEY,                -- agent 名，如 a1/a2/a3
    session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE,
    provider TEXT NOT NULL,             -- codex / claude / gemini / ...
    state TEXT NOT NULL,                -- SPAWNING / IDLE / BUSY / STUCK / PROMPT_PENDING / ...
...
- 失败回滚：事务失败 → 不动文件系统；事务成功后文件系统操作失败 → 标记 state='CRASHED' 触发下次启动 reconciliation
- 采用 **can-input 物理探针**（发送无害字符并验证回显）作为 `SPAWNING → IDLE` 的法定就绪门，彻底消灭视觉假绿。

### 4.3 Reconciliation Loop（启动时调谐）

ccbd 启动时强制执行：

1. 读 SQLite，列出所有处于活跃状态（`SPAWNING`, `WAITING_FOR_ACK`, `BUSY`, `IDLE`, `PROMPT_PENDING`）的 agents。
2. 比对：
   - 进程是否真的存在（`/proc/<pid>` 检查）
   - tmux pane 是否真的存活（`tmux list-panes`）
3. 不一致：
   - DB 说活、实际死 → 标记 `CRASHED`，触发资源清理。
   - DB 说死、实际活 → 标记 `orphan`，发送 SIGTERM/SIGKILL。
4. 同时扫文件系统孤儿：
   - `~/.local/state/ccb-rs/<project_id>/sandboxes/` 中没有对应 `agents` 行的 → 标记可清理。

**这就是 Janitor 调谐逻辑**。和主业务逻辑使用同一个数据源。

### 4.4 Agent 抽象

在 ccbd 层面，一个 agent = `(Managed Process, IPC Channel, PTY, PromptHandler)`。**ccbd 具备基础的交互式 Prompt 识别能力**，它知道："这是一个挂在 tmux pane 里的进程，它现在被一个 'Trust Workspace' 的弹窗卡住了"。

ccbd 职责：
- `output_chunk` 事件（同步 stdout 流）
- 识别并处理交互式弹窗（通过 `prompt_handler` 模块）
- `STUCK` 事件（超过阈值无输出抖动）
...
```
~/.local/state/ccb-rs/<project_id>/ccbd.sock  (prod)
...
| Method | Params | 用途 |
|---|---|---|
| `session.create` | `{project_id, master_pid}` | 申请新 session |
| `session.kill` | `{session_id}` | 关闭 session (级联 kill) |
| `session.list` | `{}` | 列出所有 session 状态 |
| `session.spawn_master_pane` | `{session_id}` | 在会话中启动主控窗 |
| `agent.spawn` | `{session_id, agent_id, provider, cwd}` | 启动新 agent |
| `agent.kill` | `{agent_id}` | 强制停止 agent |
| `agent.send` | `{agent_id, text}` | 向 stdin 发送字节 |
| `agent.read` | `{agent_id, since_seq_id}` | 拉取输出事件流 |
| `agent.resolve_prompt` | `{agent_id, action}` | 人工裁决弹窗 |
| `agent.watch` | `{agent_id}` | 监听进程退出 |
| `agent.assert_state` | `{agent_id, expected_state}` | 状态断言 |
| `agent.discard_evidence` | `{agent_id}` | 清理调试证据 |
| `job.submit` | `{agent_id, prompt_text}` | 提交异步任务 |
| `job.wait` | `{job_id}` | 等待任务结果 |
| `job.cancel` | `{job_id}` | 取消排队任务 |
| `system.dump` | `{}` | 调试快照 |
| `system.shutdown` | `{}` | 安全关闭 daemon |
...
**初期决策：方案 A（保留 tmux）**。tmux 是稳定的成熟工具，不必现在重新发明。但 ccbd 统一封装了对 `std::process::Command("tmux")` 的调用，**杜绝 shell 命令字符串拼接**，保证参数传递的安全性。
...
| **ccbd 状态目录** | `~/.local/state/ccb-rs/<project_id>/` | SQLite db、socket、ccbd 自己的日志 |
| **ccbd 缓存** | `~/.cache/ccb-rs/sandboxes/<short_hash>/` | 每个项目的 agent 沙盒 HOME（auth symlink + 隔离的 history）|
...
pub fn resolve_state_layout(request: &StateLayoutRequest) -> StateLayout {
    ...
    if std::env::var("CCB_ENV").as_deref() == Ok("dev") {
        return StateLayout {
            state_dir: repo_root().join("target/dev_state"),
            ...
        };
    }
    // ... 基于项目 ccb.toml 位置的哈希路径 ...
}
```

开发：`CCB_ENV=dev cargo run`，所有运行态产物都在 `target/dev_state/` 下，可以放心 `cargo clean` 重置。
部署：`cargo build --release`，二进制复制到 `~/.local/bin/ccb-rs`，运行时自动按项目隔离。
...
| Q6 | tmux 操作方式 | 封装 `std::process::Command` (MVP2 已落地) | M2 |
