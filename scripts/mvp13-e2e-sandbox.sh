#!/usr/bin/env bash
# mvp13 default isolation e2e: spawn 3 agent + ask + verify reply 干净 + kill
# 跟 mvp13-e2e-no-sandbox.sh 区别: 不 set CCBD_UNSAFE_NO_SANDBOX。
# PR2 T4 后不再走 bwrap；隔离模型是 provider HOME/env 重定向 + OAuth symlink，
# 进程回收仍由 systemd-run --scope 负责。
...
provider = "claude"
EOF
echo "  test config: 3 agents (codex / gemini / claude, master disabled, default isolation/no-bwrap)"
...
if echo "$ASK_OUT" | grep -qE "from isolated a1|reply"; then
  ...
  echo "  WARNING: ask reply unclear or missing 'from isolated a1'"
fi
...
echo ""
echo "=== default isolation e2e DONE ==="
