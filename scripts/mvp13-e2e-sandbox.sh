...
cleanup() {
  echo ""
  echo "=== cleanup trap ==="
  SESSION_ID=$(CCB_ENV=dev ./target/release/ccb-rust ps 2>&1 | grep -oE 'sess_[a-f0-9-]+' | head -1)
  if [ -n "$SESSION_ID" ]; then
    CCB_ENV=dev ./target/release/ccb-rust kill --session "$SESSION_ID" 2>&1 | head -3 || true
  fi
  sleep 1
...
START_OUT=$(CCB_ENV=dev ./target/release/ccb-rust --config "$TEST_CONFIG" start --wait 2>&1)
...
ASK_OUT=$(CCB_ENV=dev timeout 90 ./target/release/ccb-rust ask a1 "echo from sandbox a1" --wait 2>&1 || echo "TIMEOUT_OR_ERROR")
...
CCB_ENV=dev ./target/release/ccb-rust kill --session "$SESSION_ID" 2>&1 | head -3
sleep 2
