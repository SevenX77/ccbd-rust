...
**⚠️ Capability holdout — master provider is CLAUDE-ONLY at the spawn site (owner: `rpc::handlers::sessions::prepare_master_pane_plan`, obs #57/#58, 2026-07-13):** the `master.provider` config field is honored by the *surrounding* modules — `cli::config` (validation/canonicalization), `cli::bundle` (bundle inspection, `bundle.rs:165`), `rpc::handlers::master_cutover` (readiness Ack/Probe), and `tests/builtin_skills.rs` even asserts master builtin skills `for_all_wired_providers` — but it is **never fed into the load-bearing spawn scaffolding**. `prepare_master_pane_plan` hardcodes `"claude"` for home/bundle/hook/credential materialization, and `SpawnMasterPaneParams` has **no `provider` field** (contrast `agent.spawn`, which threads `manifest.provider_name` dynamically — `agent.rs:177`). `git log -S 'master.provider'` confirms it was **never** wired into `prepare_home_layout*`/`resolve_bundles_for_provider` in any revision — this is unfinished wiring, not a regression of a working path. Consequences: (1) `config validate` accepts `[master] provider="codex"` (**假绿**) then `session.spawn_master_pane` dies with the opaque `ENVIRONMENT_NOT_SUPPORTED` (worsened by the #151 fail-closed shared-credentials check, which turns the latent claude-pin into a hard error when `[providers.claude]` is absent); (2) config-time bundle validation checks against `master.provider`'s rules while runtime materializes claude's — internally inconsistent. **Any work that touches master spawn/provider MUST**: parameterize `prepare_master_pane_plan` (add `provider` to `SpawnMasterPaneParams`, have `start.rs`/cutover send `master.provider`, route per-provider home/bundle/hook/credentials like `agent.spawn`), **AND** add an end-to-end lock test driving a non-claude master through spawn (asserting codex home/bundle/creds are materialized) — because today **no test locks the spawn layer**, only the surrounding shell. Until parameterized, `config validate` should warn (not silently accept) `master.provider != claude`.
