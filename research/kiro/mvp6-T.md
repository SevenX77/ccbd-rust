...
### T0.4: ccb ps 子命令 + tabled 输出（Round 3 修订：hash 必须用 compute_socket_name）

* **依赖前置**: T0.3
* **设计输入**: `mvp6-D.md §3.4 + §4.1.1`
...
* **执行步骤**:
  ...
  5. **底部 hint 必须调用 `ccbd::tmux::compute_socket_name(&state_dir)`**（Round 3 反馈采纳，不允许 hardcode），其中 state_dir 通过 socket_path.parent() 反推。这样 daemon 端 TmuxServer::new 跟 ccb ps hint 输出的 socket name 完全一致——保证 attach 命令可用
  6. 输出格式：`💡 To inspect agents live: tmux -L <socket_name> attach -t ccbd-agents`
* **独立验收**: 同 T0.3 但跑 `./target/release/ccb ps`，输出表格 + hint；hint 中的 socket_name 跟 daemon 实际跑的 tmux server socket 一致（手动 `tmux -L <socket> ls` 应该能看到）
...
### T2.3: 切除 src/pty/ + 改造剩余 portable_pty 引用 + Cargo.toml 移除依赖（Round 3 修订：扩展覆盖 sandbox + monitor）

* **依赖前置**: T2.2
* **设计输入**: `mvp6-D.md §6.1 + §9.1 + Round 3 反馈：sandbox/systemd.rs 和 monitor/agent_watch.rs 还有 portable_pty 引用`
* **输出产物**: src/pty/ 删除；Cargo.toml 移除 portable-pty 依赖；src/sandbox/systemd.rs + src/monitor/agent_watch.rs 改造
* **执行步骤**:
  1. **完整 grep 当前所有 portable_pty 引用**（不限 src/pty/）：
     ```bash
     grep -rnE 'portable_pty|portable-pty|CommandBuilder' src/ tests/ Cargo.toml --include='*.rs' --include='*.toml'
     ```
     已知至少：
     - `src/sandbox/systemd.rs:4` `use portable_pty::CommandBuilder;`
     - `src/sandbox/systemd.rs:42` `argv_strings(cmd: &portable_pty::CommandBuilder)`
     - `src/monitor/agent_watch.rs:12` `child: Box<dyn portable_pty::Child + Send + Sync>`
  2. **改造 src/sandbox/systemd.rs**：
     - `argv_strings` 入参从 `&CommandBuilder` 改为 `&[String]`（Vec 切片）
     - 调用方（应该是 spawn handler 内部）改为先把 cmd 拼成 `Vec<String>` 再传
     - 移除 `use portable_pty::CommandBuilder;`
  3. **改造 src/monitor/agent_watch.rs::spawn_agent_pidfd_watch_task 签名**：
     - 移除 `child: Box<dyn portable_pty::Child + Send + Sync>` 参数（pidfd watch 只需要 pid + pidfd，不需要 child handle 来 wait）
     - caller（handlers.rs::handle_agent_spawn）相应不再传 child
     - watch 内部用 pidfd_open 拿到的 fd + tokio::io::AsyncRead 监听 EPOLLIN，跟 mvp2 现状的 pidfd_close 路径一致
     - 注意：pidfd 退出码获取改用 `nix::sys::wait::waitid` + `WPidFd` 而非通过 child handle（mvp2 已有这个机制）
  4. **改造调用方所有 spawn_agent_pidfd_watch_task 调用点**：移除传 child 参数
  5. **改造 mvp2/3/4 acceptance**：如果有测试直接调 spawn_agent_pidfd_watch_task 也要改签名
  6. `rm -rf src/pty/`
  7. `src/lib.rs` 删除 `pub mod pty;`
  8. Cargo.toml 删 `portable-pty = "0.8.1"`
  9. `cargo build` 通过
* **独立验收**:
  - `grep -rnE 'portable-pty|portable_pty|CommandBuilder' Cargo.toml src/ tests/` 返回 0 行
  - `cargo build` 通过
  - `cargo test --lib` 全绿
  - `Cargo.lock` 不再含 portable-pty 及其传递依赖
