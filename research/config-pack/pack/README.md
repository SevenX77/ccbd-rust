...
- `OPERATOR-HANDBOOK.md` —— **operator 手册(角色内核 + 从接管到跑通)**:角色与判断内核(定位=用户代理 CEO、六项职责、组织架构、四问判断框架)+ 四个场景细则(业务 SOP 断点/运行时故障/汇报与升级/审计与保留权力)+ 项目特殊文档接口 + 操作路径(接管四步、从零拉栈、派单文件注入 SOP、哨兵驾驶、收口善后)。被任命为 operator 的 agent 从这里上岗。
...
注:项目特有文档(用户目标与原则、需求/模块/疗效台账、观察日志、应急案例)不属于本包,由各项目按 `OPERATOR-HANDBOOK.md`「项目特殊文档接口」在自己仓内落盘,并在项目级说明文件中声明实际路径。
